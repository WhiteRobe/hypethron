...
async function GET_userAccounts(ctx, next) {
  ...
  try {
    ...
    if (uid > 0) { // 精确筛选模式
      ...
      let sql = 'true';
      if (filter.username) {
        for (let i = 0; i < 3; i++) values.push(`%${filter.username}%`);
        sql += ' AND (username LIKE ? or account LIKE ? or openid LIKE ?)';
      }
      ...
      let res = await mysql.query(
        `SELECT * FROM user_account WHERE ${sql};`, values
      ).catch(err => {
        throw err;
      });
  ...
  try {
    ...
    try { // 开始进行事务
      ...
      await connection.query( // 插入用户账户表
        ...
        [nextUid, username, account, hmac(SERVER_SALT, password, {alg: "md5", repeat: 1}), salt]
      ).catch(err => {
          throw err;
        }
      );
...
async function PUT_userAccounts(ctx, next) {
  ...
  try {
    ...
    if (!data.username || !data.account || !data.openid || !data.password || !data.salt || !data.authority) {
      throw new TypeError("缺失必要的参数，请尝试调用PATCH动作.");
    }

    let res = await mysql.query(
      'UPDATE user_account SET username=?, account=?, openid=?, password=?, salt=?, authority=? WHERE uid=?;',
      [data.username, data.account, data.openid, hmac(SERVER_SALT, data.password, {
        alg: "md5",
        repeat: 1
      }), data.salt, data.authority, uid]
    ).catch(err => {
      throw err;
    });
